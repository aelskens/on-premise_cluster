...
if $gpu; then
  ...
  kubectl label nodes $remotehostname feature.node.kubernetes.io/pci-10de.present=true
  # On some Tegra-based systems NFD detects the CPU vendor ID as NVIDIA
  kubectl label nodes $remotehostname feature.node.kubernetes.io/cpu-model.vendor_id="NVIDIA"
  # We allow a GPU deployment to be forced by setting the following label to "true"
  kubectl label nodes $remotehostname nvidia.com/gpu.present=true
fi